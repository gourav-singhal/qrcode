...
import { Modal, Picker, DatePickerIOS, Platform } from 'react-native';
    ...
    {Platform.OS === 'ios' || props.type === 'datetime' ? (
      <View>
        <Input
          ...
          onFocus={
            Platform.OS === 'android' && props.type === 'datetime'
          ...
          value={
            props.type === 'datetime'
              ? props.inputValue &&
                moment(props.inputValue).format('DD-MM-YYYY hh:mm')
              : props.inputValue
          }
        />
        <Modal
              ...
              >
                {props.items &&
                  props.items.map(item => (
                    <Picker.Item key={item.id} {...item} />
                  ))}
              </Picker>
            )}
            {Platform.OS === 'ios' && (
              <Button onPress={props.toggleModal} style={{ marginTop: 20 }}>
                Save
              </Button>
            )}
          </View>
      ...
      <View row>
        {props.items &&
          props.items.map((item, index) => (
            ...
            <View
              flex
              marginR-20={props.items.length - 1 !== index}
              key={item.id}
            >
              <Button
                radius={5}
                ...
                variant={
                  props.inputValue === item.value ? 'darkBlue' : 'lightGray'
                }
              >
