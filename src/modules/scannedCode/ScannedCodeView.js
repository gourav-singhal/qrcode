...
export const ShareButton = (props: { data: any }) => (
  <Button radius={5} onPress={() => Share.share({ message: props.data })}>
    Share
  </Button>
);
export const CopyButton = (props: {
  ...
  <Button radius={5} onPress={() => props.copyToClipboard(props.data)}>
    Copy
  </Button>
);
export const OpenButton = (props: { data: any, children?: string }) => (
  <Button radius={5} onPress={() => openLink(props.data)}>
    {props.children || 'Open'}
  </Button>
);
...
export const OpenInMaps = (props: { longitude: number, latitude: number }) => (
  <Button
    radius={5}
    onPress={() =>
      openMap({ latitude: props.latitude, longitude: props.longitude })
    }
  >
    Open in Maps
  </Button>
        ...
        <View paddingH-20>
          {parsedString.type === codeTypesList.EVENT && (
            <View>
              <Button
                radius={5}
                onPress={() => props.addToCalendar(fieldsDict)}
              >
                Add to Calendar
              </Button>
            </View>
          ...
          {parsedString.type === codeTypesList.EMAIL && (
            <View>
              <OpenButton
                ...
                data={`mailto:${fieldsDict.to}?subject=${fieldsDict.subject}&body=${fieldsDict.body}`}
              >
                Send
              </OpenButton>
            </View>
          ...
          {parsedString.type === codeTypesList.CONTACT && (
            <View row>
              ...
              <View flex marginR-20>
                <OpenButton data={`mailto:${fieldsDict.email}`}>
                  Email
                </OpenButton>
              </View>
              <View flex>
                <Button
                  radius={5}
                  onPress={() => props.addToContacts(fieldsDict)}
                >
                  Save
                </Button>
              </View>
          ...
          {parsedString.type === codeTypesList.SMS && (
            <View>
              <OpenButton
                data={`sms:${fieldsDict.to}${Platform.select({
                  ios: '&',
                  android: '?',
                })}body=${fieldsDict.message}`}
              >
                Send
              </OpenButton>
            </View>
          ...
          {parsedString.type === codeTypesList.GEO && (
            <View>
              <OpenInMaps
                longitude={Number(fieldsDict.longitude)}
                latitude={Number(fieldsDict.latitude)}
              />
            </View>
          )}
          <TouchableOpacity
            ...
            onPress={
              props.isPro ? props.goGeneratedCodePage : props.goPricingPage
            }
          >
            <Text default gray>
              {i18n.t('screens.scanned.show')}&nbsp;
            </Text>
            {!props.isPro && (
              <View
