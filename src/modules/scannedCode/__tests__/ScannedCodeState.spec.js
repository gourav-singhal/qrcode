...
describe('parseScannedString', () => {
  ...
  it('should scan wifi correctly', () => {
    expect(
      parseScannedString(
        'WIFI:S:WIFI_SSID;T:WIFI_ENCRYPTION;P:WIFI_PASSWORD;;',
      ),
    ).toEqual({
  ...
  it('should scan email correctly', () => {
    expect(
      parseScannedString(
        'mailto:mail@example.com?subject=Your Subject&body=Body paragraph',
      ),
    ).toEqual({
    ...
    expect(
      parseScannedString(
        'mailto:mail@example.com?body=Body paragraph&subject=Your Subject',
      ),
    ).toEqual({
      type: codeTypesList.EMAIL,
      fields: [
    ...
    expect(
      parseScannedString('mailto:mail@example.com?subject=Your Subject'),
    ).toEqual({
      type: codeTypesList.EMAIL,
      fields: [
        {
    ...
    expect(
      parseScannedString('mailto:mail@example.com?body=Body paragraph'),
    ).toEqual({
      type: codeTypesList.EMAIL,
      fields: [
        {
...
;`),
  ...
  it('should scan links correctly', () => {
    ...
    expect(
      parseScannedString('https://subdomain.domain.com?prop=one&prop2=two'),
    ).toEqual({
      type: codeTypesList.URL,
      fields: [
        {
  ...
  it('should scan contact correctly', () => {
    expect(
      parseScannedString(`BEGIN:VCARD
VERSION:4.0
...
END:VCARD`),
  ...
  it('should scan event correctly', () => {
    expect(
      parseScannedString(`BEGIN:VEVENT
SUMMARY:EVENT_TITLE
...
describe('ScannedCodeReducer', () => {
  it('should return the same state on random action', () => {
    expect(ScannedCodeReducer({ one: 'two' }, { type: 'random' })).toEqual({
      one: 'two',
    });
  });
});
